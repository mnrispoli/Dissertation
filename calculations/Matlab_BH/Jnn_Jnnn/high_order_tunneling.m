...
% disorder strengths
NW=40;
Ws=linspace(0.25,10,NW) % units of J
%Ws=Ws.*0;

% tupe of disorder
isQP=1; % 0 = uniform random, 1 = FFT of disorder from exp, 2 = actual QP
NB=1;%reuse this to determine all Jn contributions
...
NSites=21 % number of sites

% num of disorders and time scans
ND=1; %disorder number
NT=50; % time steps
Ts=linspace(0,NSites/10,NT); %actual times for ED evaluation 

% periodic boundary conditions
isPB = 0; % 0 = open boundary; 1 = periodic boundary
jn = 5; % order of neighbor tunnelings
...
if exist(edFileName,'file')==2
    ...
    Hj=zeros(HilbD,HilbD,jn);
    for jni=1:jn
        % if it doesn't exist then make them all
        [Hi,Hj(:,:,jni),basis] = MakeHamiltoniansAndBasis(NSites,NPart,jni,isPB); 
    end
...
Jall=csvread('BHJ0.csv')

Jint1 = @(v) interp1(Jall(1,:),Jall(2,:),v)
Jint2 = @(v) interp1(Jall(1,:),Jall(3,:),v)
Jint3 = @(v) interp1(Jall(1,:),Jall(4,:),v)
Jint4 = @(v) interp1(Jall(1,:),Jall(5,:),v)
Jint5 = @(v) interp1(Jall(1,:),Jall(6,:),v)


%% some stuff to determine before fitting
nsh=(NSites-1)/2;
sites=-nsh:nsh;
...
[XX,SS]=meshgrid(Ts',sites);



fb = fittype(@(J,F,x1,x2,z) abs(besselj(abs(x2), (4.*J./F).*sin(2.*pi.*F.*x1./2))).^2,'coefficients',{'J','F'},'independent',{'x1','x2'},'dependent','z');
%fb = fittype(@(J,x1,x2,z) abs(besselj(abs(x2), 4*pi*J*x1)).^2,'coefficients',{'J'},'independent',{'x1','x2'},'dependent','z');
yyy=nbar;
%fitted2=fit([XX(:),SS(:)],yyy(:),fb,'StartPoint',[J1./1.19]);
fitted2=fit([XX(:),SS(:)],yyy(:),fb,'StartPoint',[J1 1E-5]);
%(confint(fitted2,0.6827))
fvals(ww,bb,:)=coeffvalues(fitted2);
errs(ww,bb,:)=mean(abs((confint(fitted2,0.6827))-ones(2,1)*coeffvalues(fitted2)),1);
...
%%
QWB = @(J,F,x1,x2) abs(besselj(abs(x2), (4.*J./F).*sin(2.*pi.*F.*x1./2))).^2
QW = @(J,x1,x2,z) abs(besselj(abs(x2), 4*pi*J*x1)).^2


figure(1)
for ww=1:10
subplot(3,4,ww)
imagesc(nbarAllSave{ww,1,1})
end

figure(2)
hold off
%there is a weird dependence on system size I don't understand yet
%dFac = 1.5095
dFac = 2.2277

plot(Ws,fvals(:,1,1)*1000./dFac)
hold on
plot(Ws,fvals(:,2,1)*1000./dFac)

plot(Ws,fvals(:,3,1)*1000./dFac)

plot(Ws,fvals(:,4,1)*1000./dFac)

plot(Ws,fvals(:,5,1)*1000./dFac)

plot(Ws,Jint1(Ws)*1000,'k--')
