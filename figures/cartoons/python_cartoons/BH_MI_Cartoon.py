...
aharm=1.0/20
yy=(np.cos(xx*2*np.pi/1.005)+1)/2+(aharm)*xx**2
...
aHO=aharm*atomLocs**2

for ii in range(NS):
    addGSL(E1+aHO[ii],atomLocs[ii],LG1,ax) 

for ii in range(NS-2):    
    atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[ii+1],E1+aHO[ii+1],dw_shdw,dw_shdw,0)
    ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
        edgecolor='white', linewidth=0, zorder=10)

    atom_x, atom_y = atomSolid(atomLocs[ii+1],E1+aHO[ii+1],dw_atom,dw_atom,0)
    ax.fill(atom_x, atom_y, alpha=0.8, facecolor = atomGreen, \
        edgecolor=atomGreen, linewidth=0, zorder=12)
    
for ii in range(NS-6):
    addGSL(E2+aHO[ii+3],atomLocs[ii+3],LG2,ax) 
    
for ii in range(NS-6):    
    atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[ii+3],E2+aHO[ii+3],dw_shdw,dw_shdw,0)
    ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
        edgecolor='white', linewidth=0, zorder=10)
    
    atom_x, atom_y = atomSolid(atomLocs[ii+3],E2+aHO[ii+3],dw_atom,dw_atom,0)
    ax.fill(atom_x, atom_y, alpha=0.8, facecolor = atomGreen, \
        edgecolor=atomGreen, linewidth=0, zorder=12)
