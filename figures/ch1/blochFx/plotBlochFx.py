...
BS0 = np.genfromtxt('data/BS0_'+str(Vo)+'_Er.csv', delimiter = ',')
BS1 = np.genfromtxt('data/BS1_'+str(Vo)+'_Er.csv', delimiter = ',')
BS2 = np.genfromtxt('data/BS2_'+str(Vo)+'_Er.csv', delimiter = ',')
BS3 = np.genfromtxt('data/BS3_'+str(Vo)+'_Er.csv', delimiter = ',')
...
ax1.plot(xx,np.real(bf1[0,::])/3+3,'-', color = maticaB)
ax1.plot(xx,np.imag(bf1[0,::])/3+3,'--', color = maticaB)

ax1.plot(xx,np.real(bf1[1,::])/3+2,'-', color = maticaC)
ax1.plot(xx,np.imag(bf1[1,::])/3+2,'--', color = maticaC)

ax1.plot(xx,np.real(bf1[2,::])/3+1,'-', color = maticaA)
ax1.plot(xx,np.imag(bf1[2,::])/3+1,'--', color = maticaA)

ax1.plot(xx,np.real(bf1[3,::])/3+0,'-', color = maticaD)
ax1.plot(xx,np.imag(bf1[3,::])/3+0,'--', color = maticaD)
...
ax2.plot(xx,np.real(bf0[0,::])/3,'-', color = maticaB, label = 'Re[f,q]')
ax2.plot(xx,np.imag(bf0[0,::])/3,'--', color = maticaB, label = 'Im[f,q]')

ax2.plot(xx,np.real(bf0[1,::])/3+1,'-', color = maticaC, label = 'Re[f,q]')
ax2.plot(xx,np.imag(bf0[1,::])/3+1,'--', color = maticaC, label = 'Im[f,q]')

ax2.plot(xx,np.real(bf0[2,::])/3+2,'-', color = maticaA, label = 'Re[f,q]')
ax2.plot(xx,np.imag(bf0[2,::])/3+2,'--', color = maticaA, label = 'Im[f,q]')

ax2.plot(xx,np.real(bf0[3,::])/3+3,'-', color = maticaD, label = 'Re[f,q]')
ax2.plot(xx,np.imag(bf0[3,::])/3+3,'--', color = maticaD, label = 'Im[f,q]')
...
ax2.legend(frameon=False, loc=(0.05,-0.37), ncol=4)
...
ax4.plot(xx,(abs(bf0[0,::])**2)/3,'-', color = maticaB, label = '|f,q|^2')
#ax4.plot(xx,np.imag(bf0[0,::])/3,'--', color = maticaB, label = 'Im[f,q]')

ax4.plot(xx,(abs(bf0[1,::])**2)/3+1,'-', color = maticaC, label = '|f,q|^2')
#ax4.plot(xx,np.imag(bf0[1,::])/3+1,'--', color = maticaC, label = 'Im[f,q]')

ax4.plot(xx,(abs(bf0[2,::])**2)/3+2,'-', color = maticaA, label = '|f,q|^2')
#ax4.plot(xx,np.imag(bf0[2,::])/3+2,'--', color = maticaA, label = 'Im[f,q]')

ax4.plot(xx,(abs(bf0[3,::])**2)/3+3,'-', color = maticaD, label = '|f,q|^2')
#ax4.plot(xx,np.imag(bf0[3,::])/3+3,'--', color = maticaD, label = 'Im[f,q]')
...
ax4.legend(frameon=False, loc=(0.05,-0.37), ncol=4)
...
ax5.plot(xx,(abs(bf1[0,::])**2)/3+3,'-', color = maticaB)
#ax4.plot(xx,np.imag(bf0[0,::])/3,'--', color = maticaB, label = 'Im[f,q]')

ax5.plot(xx,(abs(bf1[1,::])**2)/3+2,'-', color = maticaC)
#ax4.plot(xx,np.imag(bf0[1,::])/3+1,'--', color = maticaC, label = 'Im[f,q]')

ax5.plot(xx,(abs(bf1[2,::])**2)/3+1,'-', color = maticaA)
#ax4.plot(xx,np.imag(bf0[2,::])/3+2,'--', color = maticaA, label = 'Im[f,q]')

ax5.plot(xx,(abs(bf1[3,::])**2)/3+0,'-', color = maticaD)
#ax4.plot(xx,np.imag(bf0[3,::])/3+3,'--', color = maticaD, label = 'Im[f,q]')
...
ax3.plot(qs,BS1, color = matica97D, linewidth=1, label = 'n=1', alpha=0.5)
ax3.plot(qs,BS0, color = matica97A, linewidth=1, label = 'n=0', alpha=0.5)



ax3.legend(frameon=False,loc=(1))

[XL,XH,YL,YH] = [-0.5, 0.5, -2.0, 6.0]
[XW,YT]=[XH-XL,YH-YL]
[W,H]=[XH-XL,YH-YL]
AR=H/W/1.618
...
plt.savefig('BlochFunctionsBS2.pdf')
