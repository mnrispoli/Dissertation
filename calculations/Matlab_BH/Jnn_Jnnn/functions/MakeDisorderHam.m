function [Hd, Rvec] = MakeDisorderHam(varargin)
...
elseif isQP == 1
    ...
    PhiOff=(varargin{3})*199./(2.*pi);
    
    Rvec=siteSave(9-(round(NSites/2)-1):10+(round(NSites/2)-1),round(PhiOff))';
    Hd=diag( ...
        sum((ones(HilbD,1)*Rvec).*basis,2) ...
        );
...
elseif isQP == 2
    ...
    Rvec=cos((beta*2*pi).*[0:NSites-1]+PhiOff)./2;
    Hd=diag( ...
        sum((ones(HilbD,1)*Rvec).*basis,2) ...
        );
