...
yg = (np.cos(xx*2*np.pi*(1.618)+1))/2*aoff

yy=(np.cos(xx*2*np.pi)+1)/2 + yg

ax.plot(xx,yy, linewidth = 2.5, color='k', zorder = 1)
ax.plot(xx,yg-0.01, linewidth = 2.5, linestyle = '--',color='gray', zorder = 1)
ax.axis('off')

atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[1]-0.02,E1+aoff,dw_shdw,dw_shdw,0)
ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
        edgecolor='white', linewidth=0, zorder=10)

atom_x, atom_y = atomSolid(atomLocs[1]-0.02,E1+aoff,dw_atom,dw_atom,0)
...
addGSL(E1+aoff,atomLocs[1]-0.02,LG1,ax)    
addGSL(E1,atomLocs[2]+0.02,LG1,ax)    

#addGSL(E2,atomLocs[0],LG2,ax)   
#addGSL(E3,atomLocs[0],LG3,ax)   

a3 = patches.FancyArrowPatch((atomLocs[1]/2+atomLocs[2]/2,E1+AWH-aoff/2), (atomLocs[1]/2+atomLocs[2]/2,E1+AWH+aoff/2), zorder = 2, \
