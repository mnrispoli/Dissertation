...
atomLocs = np.linspace(0.5-NS/2,NS/2-0.5,NS)
...
def latPot(xi,tilt):
    yy=(np.cos(xi*2*np.pi/1-0.05)+1)/2+(tilt)*xi**1
    return yy

ax.plot(xx,latPot(xx,tilt), linewidth = 2.5, color='k', zorder = 1)
ax.axis('off')

#
#atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[0],E1,dw_shdw,dw_shdw,0)
...
#
#
#atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[2],E1,dw_shdw,dw_shdw,0)
#ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
#        edgecolor='white', linewidth=0, zorder=10)
#
...
# add lines for ground states
    ...
    yb=np.ones(100)*Elvl;
    axIn.plot(xb,yb, linewidth = 2, alpha = 0.5, color='gray', zorder = -10)
 
aHO=tilt*atomLocs**1

for ii in range(NS):
    addGSL(E1+aHO[ii],atomLocs[ii],LG1,ax) 

for ii in range(NS/2):    
    atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[ii*2],E1+aHO[ii*2],dw_shdw,dw_shdw,0)
    ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
        edgecolor='white', linewidth=0, zorder=10)

    atom_x, atom_y = atomSolid(atomLocs[ii*2],E1+aHO[ii*2],dw_atom,dw_atom,0)
    ax.fill(atom_x, atom_y, alpha=0.8, facecolor = atomGreen, \
        edgecolor=atomGreen, linewidth=0, zorder=12)
    
for ii in range(NS/2):    
    atom_x, atom_y = atomSolid(atomLocs[ii*2-1],E1+aHO[ii*2-1],dw_atom,dw_atom,0)
...
for ii in range(NS/2):    
    ...
    atom_x, atom_y = atomSolid(atomLocs[ii*2],E2+aHO[ii*2],dw_atom,dw_atom,0)
    ax.fill(atom_x, atom_y, alpha=0.4, facecolor = atomGreen, \
        edgecolor='k', linewidth=1, linestyle='--', \
        zorder=12)
    
for ii in range(NS):
    addGSL(E2+aHO[ii],atomLocs[ii],LG2,ax) 
    
#for ii in range(NS):    
#    atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[ii],E2+aHO[ii],dw_shdw,dw_shdw,0)
#    ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
#        edgecolor='white', linewidth=0, zorder=10)
#    
#    atom_x, atom_y = atomSolid(atomLocs[ii],E2+aHO[ii],dw_atom,dw_atom,0)
#    ax.fill(atom_x, atom_y, alpha=0.8, facecolor = atomGreen, \
#        edgecolor=atomGreen, linewidth=0, zorder=12)
