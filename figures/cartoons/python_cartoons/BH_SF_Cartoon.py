...
def latPot(xi,aharm):
    yy=(np.cos(xi*2*np.pi/1.01)+1)/2+(aharm)*xi**2
    return yy

ax.plot(xx,latPot(xx,aharm), linewidth = 2.5, color='k', zorder = 1)
ax.axis('off')
...
LGList=[LG1+0.00,LG1+0.08,LG1+0.14,LG1+0.18,\
        LG1+0.18,LG1+0.14,LG1+0.08,LG1+0.00]
for ii in range(NS-2):    
#    atom_shdw_x, atom_shdw_y = atomShadow(atomLocs[ii+1],E1+aHO[ii+1],dw_shdw,dw_shdw,0)
#    ax.fill(atom_shdw_x, atom_shdw_y, alpha=1, facecolor = 'white',
#        edgecolor='white', linewidth=0, zorder=10)
    
    xr,yr=retGSL(0.8,atomLocs[ii+1],LGList[ii],ax)
    ax.fill_between(xr, latPot(xr,aharm), yr,alpha=0.8, facecolor = atomGreen)
